...
$RAUniforme = $_POST['RAUniforme'];
$MotivoUniforme = $_POST['MotivoUniforme'];
$DataUniforme = $_POST['DataUniforme'];



$sqlinsert="INSERT INTO tb_uniforme(id_uniforme, ra, motivo_uniforme, data_uniforme) VALUES('', '$RAUniforme', '$MotivoUniforme', '$DataUniforme')";

/* Realiza uma query MySQL em caso de erro exibe a mensagem*/
mysql_query($sqlinsert) or die ("Não foi possível inserir");

echo " </br><p class=\"lead\">Falta de uniforme cadastrado com sucesso!</p> ";

$sqlaviso=mysql_query("SELECT count(id_uniforme) as uniformes from tb_uniforme where ra ='".mysql_real_escape_string($RAUniforme)."'");
$data=mysql_fetch_assoc($sqlaviso);
//echo $data['uniformes'];

if(($data['uniformes'] % 3) == 0){
  
  ?>
      <div class="alert alert-danger" role="alert">
        <strong>3 FALTAS DE UNIFORME!!!</strong> Favor entrar em contato com os responsáveis do aluno.
      </div>
<?php


$sqlselect="SELECT * from tb_uniforme where ra ='".mysql_real_escape_string($RAUniforme)."' ORDER BY data_uniforme DESC";
$sqlselecta="SELECT * from tb_aluno where ra ='".mysql_real_escape_string($RAUniforme)."'";
