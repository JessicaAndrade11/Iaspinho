...
$sqlselect = "SELECT * FROM tb_usuario where senha = '$Senha' and login ='$LoginUsuario'";
$limite = mysql_query($sqlselect);
//$resultado = mysql_fetch_assoc($limite);
...
if (mysql_num_rows($limite) <= 0){
 ...
 while ($sqlselect = mysql_fetch_array($limite)){
   ...
   $acesso = $sqlselect["adm"];

    
}
  if($acesso == 1){

    echo '<script>location.href="HomeAdm.html";</script>';
